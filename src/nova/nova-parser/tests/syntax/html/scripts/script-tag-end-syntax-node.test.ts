...
test("ScriptTagEndSyntaxNode: constructs with children and trivia", (context: TestContext) => {
    const child1 = new TestNode();
    const child2 = new TestNode();
    const trivia = new TriviaSyntaxNode(" ", null!);
    const node = new ScriptTagEndSyntaxNode([child1, child2], trivia);
...
test("ScriptTagEndSyntaxNode: children property is readonly", (context: TestContext) => {
    const child = new TestNode();
    const node = new ScriptTagEndSyntaxNode([child]);
