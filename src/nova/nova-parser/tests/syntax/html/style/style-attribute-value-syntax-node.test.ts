...
test("StyleAttributeValueSyntaxNode: constructs with children and trivia", (context: TestContext) => {
    const child1 = new TestNode();
    const child2 = new TestNode();
    const trivia = new TriviaSyntaxNode(" ", null!);
    const node = new StyleAttributeValueSyntaxNode([child1, child2], trivia);
...
test("StyleAttributeValueSyntaxNode: children property is readonly", (context: TestContext) => {
    const child = new TestNode();
    const node = new StyleAttributeValueSyntaxNode([child]);
