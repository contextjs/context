...
test("CompositeSyntaxNode: constructs with children and trivia", (context: TestContext) => {
    const child1 = new TestNode();
    const child2 = new TestNode();
    const trivia = new TriviaSyntaxNode(" ", null!);
    const node = new ConcreteCompositeNode([child1, child2], trivia);
...
test("CompositeSyntaxNode: children property is readonly", (context: TestContext) => {
    const child = new TestNode();
    const node = new ConcreteCompositeNode([child]);
...
test("CompositeSyntaxNode: children array is the same instance", (context: TestContext) => {
