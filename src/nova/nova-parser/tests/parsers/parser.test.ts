...
test("Parser: whitespace only returns literal node with trivia", (context: TestContext) => {
    ...
    context.assert.strictEqual(result.nodes[0].value, "   \t\r\n  ");
    context.assert.strictEqual(result.diagnostics.length, 0);
...
test("Parser: trivia before and after tag", (context: TestContext) => {
    ...
    const literal2 = result.nodes[2] as LiteralSyntaxNode;
    context.assert.strictEqual(literal1.value, "  ");
...
test("Parser: invalid tag emits diagnostic and recovers", (context: TestContext) => {
    ...
    context.assert.strictEqual(tag.children[0].children[2].children[0].children[0].value, "<span");
    context.assert.strictEqual(result.diagnostics[0].message.code, DiagnosticMessages.MismatchedEndTag("div", "span").code);
    context.assert.strictEqual(result.diagnostics[0].message.message, DiagnosticMessages.MismatchedEndTag("div", "span").message);
});
...
test("Parser: invalid attribute emits diagnostic and recovers", (context: TestContext) => {
    ...
    context.assert.strictEqual(attr2.children[0].children[0].value, "foo");
