...
test("TagParser: parses tag with trivia after closing", (context: TestContext) => {
    ...
    context.assert.ok(startTag.children[2].trivia instanceof TriviaSyntaxNode);
    context.assert.strictEqual(startTag.children[2].trivia!.value, "   ");
});
...
test("TagParser: preserves whitespace between attributes", (context: TestContext) => {
    const { node } = parseTag('<div foo="a"   bar="b" >');
    const startTag = getStartTag(node);
    const attrs = getAttributes(startTag);

    context.assert.ok(attrs[1].trivia || attrs[1].children[0].trivia || attrs[1].children.at(-1)?.trivia);
});
...
test("TagParser: parses tag with only opening bracket and trivia", (context: TestContext) => {
    ...
    context.assert.strictEqual(startTag.children[1].children.length, 0);
    context.assert.ok(node.trivia?.value === null || node.trivia?.value === undefined || node.trivia?.value === "");
});
