...
test("TransitionParser: parses transition at beginning of input with trailing trivia", (context: TestContext) => {
    const { node, parserContext } = parseTransition("@ ");

    context.assert.strictEqual(node.value, "@");
    context.assert.ok(node.trivia);
