...
test("BraceParser: attaches trivia (whitespace) before brace", (context: TestContext) => {
    const { node } = parseBrace("   {", "{");
...
test("BraceParser: returns diagnostic for missing opening brace", (context: TestContext) => {
    ...
    context.assert.strictEqual(parserContext.diagnostics[0].message.code, DiagnosticMessages.ExpectedBrace("a").code);
    context.assert.strictEqual(parserContext.diagnostics[0].message.message, DiagnosticMessages.ExpectedBrace("a").message);
});
...
test("BraceParser: attaches trivia for tabs/newlines before brace", (context: TestContext) => {
    const { node } = parseBrace("\t\n}", "}");
...
test("BraceParser: attaches mixed whitespace/trivia before brace", (context: TestContext) => {
    const { node } = parseBrace(" \t\n{", "{");
    context.assert.ok(node.leadingTrivia?.value === " \t\n");
});