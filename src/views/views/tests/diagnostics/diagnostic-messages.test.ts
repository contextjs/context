...
test("DiagnosticMessages: NoWhitespaceAfterTransition", (context: TestContext) => {
    const diagnosticMessage = DiagnosticMessages.NoWhitespaceAfterTransition;

    context.assert.ok(diagnosticMessage instanceof DiagnosticMessage);
    context.assert.strictEqual(diagnosticMessage.code, 3003);
    context.assert.strictEqual(diagnosticMessage.message, "No whitespace allowed after transition marker.");
});

test("DiagnosticMessages: MalformedCodeBlock", (context: TestContext) => {
    const diagnosticMessage = DiagnosticMessages.MalformedCodeBlock;

    context.assert.ok(diagnosticMessage instanceof DiagnosticMessage);
    context.assert.strictEqual(diagnosticMessage.code, 3004);
    context.assert.strictEqual(diagnosticMessage.message, "Malformed code block. Expected a valid code block syntax.");
});

test("DiagnosticMessages: UnexpectedTransition", (context: TestContext) => {
    const diagnosticMessage = DiagnosticMessages.UnexpectedTransition;

    context.assert.ok(diagnosticMessage instanceof DiagnosticMessage);
    context.assert.strictEqual(diagnosticMessage.code, 3005);
