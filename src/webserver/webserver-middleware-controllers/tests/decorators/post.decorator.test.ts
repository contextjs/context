...
import { Post } from '../../src/decorators/post.decorator.js';
import { VERB_ROUTE_META } from '../../src/decorators/verb-decorator.js';

test('PostDecorator: metadata is undefined for undecorated method', (context: TestContext) => {
...
test('PostDecorator: sets correct template and POST verb metadata', (context: TestContext) => {
    const template = '/post-path';
    class DecoratedClass { action() { } }
    const descriptor = Object.getOwnPropertyDescriptor(DecoratedClass.prototype, 'action');
    Post(template)(DecoratedClass.prototype, 'action', descriptor as TypedPropertyDescriptor<any>);
    ...
    context.assert.strictEqual(meta.verb, 'POST');
});
