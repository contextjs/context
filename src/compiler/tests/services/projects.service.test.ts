...
test("ProjectsService: getParsedConfig - returns valid tsconfig", (context: TestContext) => {
    ...
    context.assert.ok(parsed.config.options);
    context.assert.strictEqual(typeof parsed.config.options.strict, "boolean");
    context.assert.ok(Array.isArray(parsed.config.fileNames));
    context.assert.ok(parsed.config.fileNames.length > 0);
});
